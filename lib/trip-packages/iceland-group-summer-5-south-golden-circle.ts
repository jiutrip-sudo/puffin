...
export const icelandGroupSummer5SouthGoldenCircle: TripPackage = {
  ...
  intro: {
    summary:
      "這個 5 天 4 夜夏季跟團套餐涵蓋黃金圈與南岸兩日，由品質小巴團與專業嚮導帶領，含冰川徒步與 Sky Lagoon 體驗。行程善用夏季長日照，節奏適中、安排務實。",
    full: `這個 5 天 4 夜夏季跟團套餐涵蓋黃金圈與南岸兩日，由品質小巴團與專業嚮導帶領。套餐含凱夫拉維克機場至市區大巴接送與 Sky Lagoon 溫泉體驗，充分利用夏季長日照與午夜陽光，行程安排務實。

第二日造訪黃金圈三大景點——辛格維利爾國家公園（Þingvellir National Park）、蓋錫爾間歇泉地帶與黃金瀑布（Gullfoss），並前往凱瑞斯火山口（Kerid Crater）。南岸動線串連塞里雅蘭瀑布（Seljalandsfoss）、斯科加瀑布（Skógafoss）、黑沙灘（Reynisfjara）、傑古沙龍冰河湖（Jökulsárlón）與鑽石沙灘（Diamond Beach）；冰川徒步由嚮導陪同並提供安全裝備（10 月前為冰川徒步，之後改為藍冰洞體驗）。`,
  },
  gallery: [
    {
