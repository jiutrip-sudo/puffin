...
export const icelandSelfDriveWinter4: TripPackage = {
  ...
  intro: {
    summary:
      "這個冬季 4 日自駕套餐是冬季自駕的首選，不僅能將冬季極致美景收入眼中，還有機會看見舞動的北極光。行程充分考量冬季日照與路況，節奏適中、安排合理。",
    full: `這個冬季 4 日自駕套餐是冬季自駕的首選，不僅能將冬季極致的美景收入眼中，還有機會能看到舞動的北極光。這個自駕遊套餐充分考慮了冬季的日照時長以及路況，令行程變得更加合理、節奏也適中。行程中的南岸路線與黃金圈路線（Golden Circle）更是冰島最熱門路線，其中冬季的傑古沙龍冰河湖（Jökulsárlón）、獨一無二的藍冰洞（Blue Ice Cave）、還有地表被白雪覆蓋的黄金瀑布（Gullfoss）也是冬季最獨特的存在。

途中除了自然景觀外，您還可以搭配選擇一些戶外項目，比如在湛藍的冰川上徒步、體驗雪地摩托帶來的速度與激情、騎著髮型飄逸的冰島馬、進入獨一無二的藍冰洞探險等眾多項目，為您的行程增加不一樣的樂趣。同時，您還可以靈活自主地掌握行程節奏，決定每一處停留的時長。

在您預訂套餐後，您還會收到一份由行程策劃師單獨為您客製的行程路書，對於沿途景點、人文、歷史等都有詳細的介紹，並且還會根據您的時間來為您設計出最適合觀看極光的路線，讓您可以輕鬆地收穫冰島南岸地區所有美景。不論是在藍湖溫泉中享受大自然的饋贈，還是在黑沙灘上聆聽地球的心跳，您都將留下獨一無二的回憶。

心動不如行動，別再猶豫，趕快來冰島體驗這絕世的美景吧，冬季還有美麗的極光就在冰島等著您！`,
  },
  gallery: [
    {
      id: "aurora",
