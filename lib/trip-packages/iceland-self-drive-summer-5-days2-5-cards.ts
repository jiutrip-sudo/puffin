...
export const icelandSelfDriveSummer5Day3OptionalActivities: TripAttraction[] = [
  {
    ...
    paragraphs: [
      ...
      "注意事項：",
      "1. 建議穿保暖防水的衣物，並穿登山靴。",
      "2. 由於冰洞的形成是純天然且人為不可控的，所以每一年冰洞的位置和樣貌都會有變化，且只能在夏季降到零度以下時才能進入。為了您更好的體驗感受，我們會根據天氣狀況做預測，如天氣狀況不符合進洞要求，我們會提前聯繫您取消行程。取消條款請參見帕芬假期旅行的服務條款。",
      "活動時長：",
      "最低參加年齡：",
      "體驗包括：",
      "不包括：",
...
export const icelandSelfDriveSummer5Day4Highlights: TripAttraction[] = [
  {
    ...
    paragraphs: [
      ...
      "Öxarárfoss是園內最受歡迎的景點之一，這條高13米、寬20米的瀑布在夏季會完全凍結，是攀冰愛好者的最愛。旅客除了通過步行路線遊覽公園，還可以體驗例如垂釣和浮潛等多種多樣的活動。公園南側的辛格瓦德拉湖是冰島境內最大的天然湖泊，湖中有褐鱒魚和各式的北極鮭魚，是全國數一數二的垂釣地點。另外，因驚人的能見度而世界聞名的絲浮拉大裂縫（Silfra）也位於國家公園範圍內。如果滿足有關潛水資格和規定，則能夠潛入清澈冰冷的地下水一探究竟。如果沒有潛水證，那麼參加帕芬假期旅行的浮潛團則是一個更好的選擇，潛水小白也能夠自在地領略絲浮拉的美麗。",
