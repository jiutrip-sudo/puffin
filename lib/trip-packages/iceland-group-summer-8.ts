...
您將造訪黃金圈三大景點——辛格維利爾國家公園（Þingvellir National Park）、蓋錫爾間歇泉地帶（Geysir）與黃金瀑布（Gullfoss），以及凱瑞斯火山口（Kerið Crater）；沿南岸探索塞里雅蘭瀑布、斯科加瀑布，參加索爾黑馬冰川徒步，並在黑沙灘（Reynisfjara）遠眺雷尼斯岩（Reynisdrangar），途中經過維克（Vík）。繼續造訪傑古沙龍冰河湖（Jökulsárlón）與鑽石沙灘（Diamond Beach）；斯奈山半島則遊覽教會山（Kirkjufell）與布迪爾黑教堂。
