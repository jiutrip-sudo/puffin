...
const SLM_HTML =
  process.env.SLM_HTML ?? path.join(ROOT, "tmp/slm-summer-group-4-live.html");
const IMG_BASE =
...
const DAY_PAIRS = [
  [
    "入境日：雷克雅未克 & Sky Lagoon舒适温泉体验",
    "塞里雅兰瀑布 & 斯科加瀑布 & 冰川徒步 & 黑沙滩",
  ],
  [
    "塞里雅兰瀑布 & 斯科加瀑布 & 冰川徒步 & 黑沙滩",
    "杰古沙龙冰河湖 & 钻石沙滩 ",
...
function renderPackage(meta, dayData) {
  ...
  const itineraryDays = meta.days.map((day, index) => {
    ...
    const hl = `icelandGroupSummer4Day${dayNum}Highlights`;
    const ex = `icelandGroupSummer4Day${dayNum}OptionalActivities`;
    return `    {
      day: ${dayNum},
      title: "${esc(title)}",
  ...
  const routeStopDetails = [
    ...
    "塞里雅蘭／斯科加瀑布 → 冰川徒步 → 黑沙灘 → 維克",
    "傑古沙龍冰河湖 → 鑽石沙灘 → 返回雷克雅未克",
    "雷克雅未克 → 機場離境",
  ];
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-4-day1-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer4Day1Highlights",
      items: dayData["入境日：雷克雅未克 & Sky Lagoon舒适温泉体验"].highlights,
    },
    {
      name: "icelandGroupSummer4Day1OptionalActivities",
      items: dayData["入境日：雷克雅未克 & Sky Lagoon舒适温泉体验"].extras,
    },
  ]),
);

const DAY2 = "塞里雅兰瀑布 & 斯科加瀑布 & 冰川徒步 & 黑沙滩";
const DAY3 = "杰古沙龙冰河湖 & 钻石沙滩 ";

fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-4-days2-4-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer4Day2Highlights",
      items: dayData[DAY2].highlights,
    },
    {
      name: "icelandGroupSummer4Day2OptionalActivities",
      items: dayData[DAY2].extras,
    },
    {
      name: "icelandGroupSummer4Day3Highlights",
      items: dayData[DAY3].highlights,
    },
    {
      name: "icelandGroupSummer4Day3OptionalActivities",
      items: dayData[DAY3].extras,
    },
    {
      name: "icelandGroupSummer4Day4Highlights",
      items: dayData["离境日"].highlights,
    },
    {
      name: "icelandGroupSummer4Day4OptionalActivities",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-4.ts"),
