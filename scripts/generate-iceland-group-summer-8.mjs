...
const SLM_HTML =
  process.env.SLM_HTML ?? path.join(ROOT, "tmp/slm-summer-group-8-live.html");
const IMG_BASE =
...
const DAY_PAIRS = [
  ["入境日", "黄金圈3+1：国家公园 & 间歇泉 & 黄金瀑布 & 火山口"],
  [
    "黄金圈3+1：国家公园 & 间歇泉 & 黄金瀑布 & 火山口",
    "南岸风光：瀑布 & 冰川徒步 & 黑沙滩",
  ],
  [
    "南岸风光：瀑布 & 冰川徒步 & 黑沙滩",
...
function renderPackage(meta, dayData) {
  ...
  const itineraryDays = meta.days.map((day, index) => {
    ...
    const hl = `icelandGroupSummer8Day${dayNum}Highlights`;
    const ex = `icelandGroupSummer8Day${dayNum}OptionalActivities`;
    return `    {
      day: ${dayNum},
      title: "${esc(title)}",
...
function fixSummer8MetaDays(decoded, days) {
  if (days.some((day) => day.title.includes("冰河湖之旅"))) return days;
  const title = "冰河湖之旅：杰古沙龙冰河湖 & 钻石沙滩";
  const match = decoded.match(
    new RegExp(
      `"title":"${title.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}","description":"((?:[^"\\\\]|\\\\.)*)"(?:,"location":(null|"([^"]*)"))?`,
    ),
  );
  const insertIdx = days.findIndex((day) =>
    day.title.includes("斯奈山半岛"),
  );
  const entry = {
    title,
    description: match?.[1]?.replace(/\\"/g, '"') ?? "",
    location: match?.[3] ?? "雷克雅未克",
  };
  if (insertIdx === -1) days.push(entry);
  else days.splice(insertIdx, 0, entry);
  return days;
}
...
meta.days = fixSummer8MetaDays(decoded, meta.days);
const outDir = path.join(ROOT, "lib/trip-packages");

fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-8-day1-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer8Day1Highlights",
      items: dayData["入境日"].highlights,
    },
    {
      name: "icelandGroupSummer8Day1OptionalActivities",
      items: dayData["入境日"].extras,
    },
  ]),
);

const DAY2 = "黄金圈3+1：国家公园 & 间歇泉 & 黄金瀑布 & 火山口";
const DAY3 = "南岸风光：瀑布 & 冰川徒步 & 黑沙滩";
const DAY4 = "冰河湖之旅：杰古沙龙冰河湖 & 钻石沙滩";
const DAY5 = "斯奈山半岛：教会山 & 黑教堂";
const DAY6 = "活动日1：火山内部 & 出海观鲸";
const DAY7 = "活动日2：自由活动日";

fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-8-days2-8-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer8Day2Highlights",
      items: dayData[DAY2].highlights,
    },
    {
      name: "icelandGroupSummer8Day2OptionalActivities",
      items: dayData[DAY2].extras,
    },
    {
      name: "icelandGroupSummer8Day3Highlights",
      items: dayData[DAY3].highlights,
    },
    {
      name: "icelandGroupSummer8Day3OptionalActivities",
      items: dayData[DAY3].extras,
    },
    {
      name: "icelandGroupSummer8Day4Highlights",
      items: dayData[DAY4].highlights,
    },
    {
      name: "icelandGroupSummer8Day4OptionalActivities",
      items: dayData[DAY4].extras,
    },
    {
      name: "icelandGroupSummer8Day5Highlights",
      items: dayData[DAY5].highlights,
    },
    {
      name: "icelandGroupSummer8Day5OptionalActivities",
      items: dayData[DAY5].extras,
    },
    {
      name: "icelandGroupSummer8Day6Highlights",
      items: dayData[DAY6].highlights,
    },
    {
      name: "icelandGroupSummer8Day6OptionalActivities",
      items: dayData[DAY6].extras,
    },
    {
      name: "icelandGroupSummer8Day7Highlights",
      items: dayData[DAY7].highlights,
    },
    {
      name: "icelandGroupSummer8Day7OptionalActivities",
      items: dayData[DAY7].extras,
    },
    {
      name: "icelandGroupSummer8Day8Highlights",
      items: dayData["离境日"].highlights,
    },
    {
      name: "icelandGroupSummer8Day8OptionalActivities",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-8.ts"),
