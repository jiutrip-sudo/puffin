...
const SLM_HTML =
  ...
  path.join(ROOT, "tmp/slm-winter-11.html");
const IMG_BASE =
...
function extractAllDays() {
  ...
  const pairs = [
    ...
    ["冰河湖风光", "东部峡湾风光"],
    ["东部峡湾风光", "米湖热能区"],
    ["米湖热能区", "众神瀑布、阿克雷里及沿途风光"],
    ["众神瀑布、阿克雷里及沿途风光", "西部沿途风光"],
    ["西部沿途风光", "斯奈山半岛"],
    ["斯奈山半岛", "首都自由行"],
    ["首都自由行", "离境日"],
    ["离境日", null],
...
fs.writeFileSync(
  path.join(outDir, "iceland-self-drive-winter-11-day1-cards.ts"),
  renderExports([
    {
      name: "icelandSelfDriveWinter11Day1Highlights",
      items: data["入境日"].highlights,
    },
    {
      name: "icelandSelfDriveWinter11Day1OptionalActivities",
...
for (let d = 2; d <= 11; d++) {
  ...
  dayExports.push(
    { name: `icelandSelfDriveWinter11Day${d}Highlights`, items: data[key].highlights },
    { name: `icelandSelfDriveWinter11Day${d}OptionalActivities`, items: data[key].extras },
  );
}

fs.writeFileSync(
  path.join(outDir, "iceland-self-drive-winter-11-days2-11-cards.ts"),
