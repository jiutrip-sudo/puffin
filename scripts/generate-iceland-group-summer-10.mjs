...
const SLM_HTML =
  process.env.SLM_HTML ?? path.join(ROOT, "tmp/slm-summer-group-10-live.html");
const IMG_BASE =
...
const DAY_PAIRS = [
  ["抵达冰岛（含接机）", "黄金圈一日游"],
  ["黄金圈一日游", "南岸瀑布 & 冰川 & 黑沙滩"],
  ["南岸瀑布 & 冰川 & 黑沙滩", "浪漫冰河湖 & 钻石沙滩"],
  ["浪漫冰河湖 & 钻石沙滩", "西部白银圈"],
  ["西部白银圈", "斯奈山半岛"],
  ["斯奈山半岛", "夏日探险：观鲸&火山"],
  ["夏日探险：观鲸&火山", "花少同款：丝浮拉浮潜&温泉"],
  ["花少同款：丝浮拉浮潜&温泉", "雷克雅未克自由活动日"],
  ["雷克雅未克自由活动日", "离境日（含送机）"],
  ["离境日（含送机）", null],
];
...
function extractPackageMeta(decoded) {
  ...
  const daysSection = decoded.slice(
    decoded.indexOf('days":[{"title":"抵达冰岛'),
  );
...
function renderPackage(meta, dayData) {
  ...
  const itineraryDays = meta.days.map((day, index) => {
    ...
    const accommodation =
      day.location && day.title !== "离境日" && !day.title.startsWith("离境日")
        ? toTw(day.location)
        : "—";
    const description = toTw(day.description);
    const hl = `icelandGroupSummer10Day${dayNum}Highlights`;
    const ex = `icelandGroupSummer10Day${dayNum}OptionalActivities`;
    return `    {
      day: ${dayNum},
      title: "${esc(title)}",
...
${attractions}
  ...
  routeOverviewSubtitle: "南岸、冰河湖、西部、斯奈山半島與花少同款體驗夏季跟團動線",
  routeStops: [
${routeStops.join(",\n")}
  ],
  routeMap: {
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-10-day1-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer10Day1Highlights",
      items: dayData["抵达冰岛（含接机）"].highlights,
    },
    {
      name: "icelandGroupSummer10Day1OptionalActivities",
      items: dayData["抵达冰岛（含接机）"].extras,
    },
  ]),
);

const DAY2 = "黄金圈一日游";
const DAY3 = "南岸瀑布 & 冰川 & 黑沙滩";
const DAY4 = "浪漫冰河湖 & 钻石沙滩";
const DAY5 = "西部白银圈";
const DAY6 = "斯奈山半岛";
const DAY7 = "夏日探险：观鲸&火山";
const DAY8 = "花少同款：丝浮拉浮潜&温泉";
const DAY9 = "雷克雅未克自由活动日";

fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-10-days2-10-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer10Day2Highlights",
      items: dayData[DAY2].highlights,
    },
    {
      name: "icelandGroupSummer10Day2OptionalActivities",
      items: dayData[DAY2].extras,
    },
    {
      name: "icelandGroupSummer10Day3Highlights",
      items: dayData[DAY3].highlights,
    },
    {
      name: "icelandGroupSummer10Day3OptionalActivities",
      items: dayData[DAY3].extras,
    },
    {
      name: "icelandGroupSummer10Day4Highlights",
      items: dayData[DAY4].highlights,
    },
    {
      name: "icelandGroupSummer10Day4OptionalActivities",
      items: dayData[DAY4].extras,
    },
    {
      name: "icelandGroupSummer10Day5Highlights",
      items: dayData[DAY5].highlights,
    },
    {
      name: "icelandGroupSummer10Day5OptionalActivities",
      items: dayData[DAY5].extras,
    },
    {
      name: "icelandGroupSummer10Day6Highlights",
      items: dayData[DAY6].highlights,
    },
    {
      name: "icelandGroupSummer10Day6OptionalActivities",
      items: dayData[DAY6].extras,
    },
    {
      name: "icelandGroupSummer10Day7Highlights",
      items: dayData[DAY7].highlights,
    },
    {
      name: "icelandGroupSummer10Day7OptionalActivities",
      items: dayData[DAY7].extras,
    },
    {
      name: "icelandGroupSummer10Day8Highlights",
      items: dayData[DAY8].highlights,
    },
    {
      name: "icelandGroupSummer10Day8OptionalActivities",
      items: dayData[DAY8].extras,
    },
    {
      name: "icelandGroupSummer10Day9Highlights",
      items: dayData[DAY9].highlights,
    },
    {
      name: "icelandGroupSummer10Day9OptionalActivities",
      items: dayData[DAY9].extras,
    },
    {
      name: "icelandGroupSummer10Day10Highlights",
      items: dayData["离境日（含送机）"].highlights,
    },
    {
      name: "icelandGroupSummer10Day10OptionalActivities",
      items: dayData["离境日（含送机）"].extras,
    },
  ]),
);

fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-10.ts"),
