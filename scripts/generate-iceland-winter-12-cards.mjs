...
const SLM_HTML =
  ...
  path.join(ROOT, "tmp/slm-winter-12.html");
const IMG_BASE =
...
fs.writeFileSync(
  path.join(outDir, "iceland-self-drive-winter-12-day1-cards.ts"),
  renderExports([
    {
      name: "icelandSelfDriveWinter12Day1Highlights",
      items: data["入境日"].highlights,
    },
    {
      name: "icelandSelfDriveWinter12Day1OptionalActivities",
...
for (let d = 2; d <= 12; d++) {
  ...
  dayExports.push(
    { name: `icelandSelfDriveWinter12Day${d}Highlights`, items: data[key].highlights },
    { name: `icelandSelfDriveWinter12Day${d}OptionalActivities`, items: data[key].extras },
  );
}

fs.writeFileSync(
  path.join(outDir, "iceland-self-drive-winter-12-days2-12-cards.ts"),
