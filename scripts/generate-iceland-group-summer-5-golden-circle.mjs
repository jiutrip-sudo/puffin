...
const SLM_HTML =
  ...
  path.join(ROOT, "tmp/slm-summer-group-5-golden-circle-live.html");
const PACKAGE_PREFIX = "icelandGroupSummer5GoldenCircle";
const PACKAGE_ID = "iceland-group-summer-5-south-golden-circle";
...
const DAY1 = "入境日：雷克雅未克 & Sky Lagoon温泉";
const DAY2 = "黄金圈3+1：辛格维利尔国家公园 & 间歇泉 & 黄金瀑布 & 凯瑞斯火山口";
const DAY3 = "塞里雅兰瀑布 & 斯科加瀑布 & 索尔黑马冰川 & 黑沙滩";
const DAY4 = "杰古沙龙冰河湖 & 钻石沙滩 & 维克镇";

fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-5-south-golden-circle-day1-cards.ts"),
  renderExports([
    {
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-5-south-golden-circle-days2-5-cards.ts"),
  renderExports([
    {
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-5-south-golden-circle.ts"),
