...
const SLM_HTML =
  process.env.SLM_HTML ?? path.join(ROOT, "tmp/slm-winter-group-4-live.html");
const IMG_BASE =
...
const DAY_PAIRS = [
  ["入境日：雷克雅未克&舒适温泉", "塞里雅兰瀑布、斯科加瀑布、黑沙滩"],
  ["塞里雅兰瀑布、斯科加瀑布、黑沙滩", "杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险"],
  ["杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险", "离境日"],
  ["离境日", null],
];
...
function extractPackageMeta(decoded) {
  const headerMatch = decoded.match(
    /"header":\{"title":"([^"]+)","images":\[([\s\S]*?)\],"days":4,"nights":3/,
  );
...
function renderPackage(meta, dayData) {
  ...
  const itineraryDays = meta.days.map((day, index) => {
    ...
    const hl = `icelandGroupWinter4Day${dayNum}Highlights`;
    const ex = `icelandGroupWinter4Day${dayNum}OptionalActivities`;
    return `    {
      day: ${dayNum},
      title: "${esc(title)}",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-winter-4-day1-cards.ts"),
  renderExports([
    {
      name: "icelandGroupWinter4Day1Highlights",
      items: dayData["入境日：雷克雅未克&舒适温泉"].highlights,
    },
    {
      name: "icelandGroupWinter4Day1OptionalActivities",
      items: dayData["入境日：雷克雅未克&舒适温泉"].extras,
    },
  ]),
);

fs.writeFileSync(
  path.join(outDir, "iceland-group-winter-4-days2-4-cards.ts"),
  renderExports([
    {
      name: "icelandGroupWinter4Day2Highlights",
      items: dayData["塞里雅兰瀑布、斯科加瀑布、黑沙滩"].highlights,
    },
    {
      name: "icelandGroupWinter4Day2OptionalActivities",
      items: dayData["塞里雅兰瀑布、斯科加瀑布、黑沙滩"].extras,
    },
    {
      name: "icelandGroupWinter4Day3Highlights",
      items: dayData["杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险"].highlights,
    },
    {
      name: "icelandGroupWinter4Day3OptionalActivities",
      items: dayData["杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险"].extras,
    },
    {
      name: "icelandGroupWinter4Day4Highlights",
      items: dayData["离境日"].highlights,
    },
    {
      name: "icelandGroupWinter4Day4OptionalActivities",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-winter-4.ts"),
