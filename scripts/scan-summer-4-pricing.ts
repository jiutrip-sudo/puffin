...
import { icelandSelfDriveSummer4Pricing } from "../lib/trip-pricing/iceland-self-drive-summer-4";
...
async function main() {
  const config = {
    ...icelandSelfDriveSummer4Pricing,
    corivo: { ...icelandSelfDriveSummer4Pricing.corivo! },
  };
    ...
    for (const accommodationTier of tiers) {
      for (const vehicleTier of vehicles) {
        try {
          ...
          console.error(`FAIL ${date} ${accommodationTier} ${vehicleTier}: ${message}`);
        }
      }
    }
  }
