...
const SLM_HTML =
  process.env.SLM_HTML ?? path.join(ROOT, "tmp/slm-summer-group-6-live.html");
const IMG_BASE =
...
const DAY_PAIRS = [
  [
    ...
    "黄金圈3+1：辛格维利尔国家公园 & 间歇泉 & 黄金瀑布 & 凯瑞斯火山口",
  ],
  [
    "黄金圈3+1：辛格维利尔国家公园 & 间歇泉 & 黄金瀑布 & 凯瑞斯火山口",
    "塞里雅兰瀑布 & 斯科加瀑布 & 索尔黑马冰川 & 黑沙滩",
  ],
  [
    "塞里雅兰瀑布 & 斯科加瀑布 & 索尔黑马冰川 & 黑沙滩",
    "杰古沙龙冰河湖 & 钻石沙滩 ",
...
function renderPackage(meta, dayData) {
  ...
  const itineraryDays = meta.days.map((day, index) => {
    ...
    const hl = `icelandGroupSummer6Day${dayNum}Highlights`;
    const ex = `icelandGroupSummer6Day${dayNum}OptionalActivities`;
    return `    {
      day: ${dayNum},
      title: "${esc(title)}",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-6-day1-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer6Day1Highlights",
      items: dayData["入境日：雷克雅未克 & Sky Lagoon温泉"].highlights,
    },
    {
      name: "icelandGroupSummer6Day1OptionalActivities",
...
const DAY2 =
  "黄金圈3+1：辛格维利尔国家公园 & 间歇泉 & 黄金瀑布 & 凯瑞斯火山口";
const DAY3 = "塞里雅兰瀑布 & 斯科加瀑布 & 索尔黑马冰川 & 黑沙滩";
const DAY4 = "杰古沙龙冰河湖 & 钻石沙滩 ";
const DAY5 = "斯奈山半岛：教会山 & 海豹沙滩 & 布迪尔黑教堂";

fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-6-days2-6-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer6Day2Highlights",
      items: dayData[DAY2].highlights,
    },
    {
      name: "icelandGroupSummer6Day2OptionalActivities",
      items: dayData[DAY2].extras,
    },
    {
      name: "icelandGroupSummer6Day3Highlights",
      items: dayData[DAY3].highlights,
    },
    {
      name: "icelandGroupSummer6Day3OptionalActivities",
      items: dayData[DAY3].extras,
    },
    {
      name: "icelandGroupSummer6Day4Highlights",
      items: dayData[DAY4].highlights,
    },
    {
      name: "icelandGroupSummer6Day4OptionalActivities",
      items: dayData[DAY4].extras,
    },
    {
      name: "icelandGroupSummer6Day5Highlights",
      items: dayData[DAY5].highlights,
    },
    {
      name: "icelandGroupSummer6Day5OptionalActivities",
      items: dayData[DAY5].extras,
    },
    {
      name: "icelandGroupSummer6Day6Highlights",
      items: dayData["离境日"].highlights,
    },
    {
      name: "icelandGroupSummer6Day6OptionalActivities",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-6.ts"),
