...
const SLM_HTML =
  process.env.SLM_HTML ?? path.join(ROOT, "tmp/slm-winter-group-5-live.html");
const IMG_BASE =
...
const DAY_PAIRS = [
  ["入境日", "冰岛南岸：塞里雅兰瀑布、斯科加瀑布、黑沙滩"],
  ["冰岛南岸：塞里雅兰瀑布、斯科加瀑布、黑沙滩", "东南部风光：杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险"],
  ["东南部风光：杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险", "黄金圈：辛格维利尔国家公园、间歇泉、黄金瀑布"],
  ["黄金圈：辛格维利尔国家公园、间歇泉、黄金瀑布", "离境日"],
  ["离境日", null],
];
...
function extractPackageMeta(decoded) {
  const headerMatch = decoded.match(
    /"header":\{"title":"([^"]+)","images":\[([\s\S]*?)\],"days":5,"nights":4/,
  );
...
function renderPackage(meta, dayData) {
  ...
  const itineraryDays = meta.days.map((day, index) => {
    ...
    const hl = `icelandGroupWinter5Day${dayNum}Highlights`;
    const ex = `icelandGroupWinter5Day${dayNum}OptionalActivities`;
    return `    {
      day: ${dayNum},
      title: "${esc(title)}",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-winter-5-day1-cards.ts"),
  renderExports([
    {
      name: "icelandGroupWinter5Day1Highlights",
      items: dayData["入境日"].highlights,
    },
    {
      name: "icelandGroupWinter5Day1OptionalActivities",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-winter-5-days2-5-cards.ts"),
  renderExports([
    {
      name: "icelandGroupWinter5Day2Highlights",
      items: dayData["冰岛南岸：塞里雅兰瀑布、斯科加瀑布、黑沙滩"].highlights,
    },
    {
      name: "icelandGroupWinter5Day2OptionalActivities",
      items: dayData["冰岛南岸：塞里雅兰瀑布、斯科加瀑布、黑沙滩"].extras,
    },
    {
      name: "icelandGroupWinter5Day3Highlights",
      items: dayData["东南部风光：杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险"].highlights,
    },
    {
      name: "icelandGroupWinter5Day3OptionalActivities",
      items: dayData["东南部风光：杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险"].extras,
    },
    {
      name: "icelandGroupWinter5Day4Highlights",
      items: dayData["黄金圈：辛格维利尔国家公园、间歇泉、黄金瀑布"].highlights,
    },
    {
      name: "icelandGroupWinter5Day4OptionalActivities",
      items: dayData["黄金圈：辛格维利尔国家公园、间歇泉、黄金瀑布"].extras,
    },
    {
      name: "icelandGroupWinter5Day5Highlights",
      items: dayData["离境日"].highlights,
    },
    {
      name: "icelandGroupWinter5Day5OptionalActivities",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-winter-5.ts"),
