...
const SLM_HTML =
  process.env.SLM_HTML ?? path.join(ROOT, "tmp/slm-winter-group-9-non-ring-live.html");
const IMG_BASE =
...
const DAY_PAIRS = [
  ["入境日", "冰岛南岸：塞里雅兰瀑布、斯科加瀑布和黑沙滩"],
  [
    "冰岛南岸：塞里雅兰瀑布、斯科加瀑布和黑沙滩",
    "东南部风光：杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险",
  ],
  [
    "东南部风光：杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险",
    "黄金圈之旅：黄金瀑布、间歇泉、冰岛议会公园",
  ],
  [
    "黄金圈之旅：黄金瀑布、间歇泉、冰岛议会公园",
    "西部小众风景：熔岩瀑布、儿童瀑布、冰岛传奇故乡",
  ],
  [
    "西部小众风景：熔岩瀑布、儿童瀑布、冰岛传奇故乡",
    "斯奈山半岛：网红黑教堂、权游箭头山、阿尔纳斯塔皮渔村",
  ],
  [
    "斯奈山半岛：网红黑教堂、权游箭头山、阿尔纳斯塔皮渔村",
    "特别浮潜日：世界热门潜水地丝浮拉裂谷浮潜体验",
  ],
  [
    "特别浮潜日：世界热门潜水地丝浮拉裂谷浮潜体验",
    "首都自由活动日：探索最北首都的悠闲时光",
  ],
  ["首都自由活动日：探索最北首都的悠闲时光", "离境日"],
  ["离境日", null],
];
...
function extractPackageMeta(decoded) {
  ...
  const daysWithSouth = insertSouthCoastDay(decoded, days);
...
function insertSouthCoastDay(decoded, days) {
  const southTitle = "冰岛南岸：塞里雅兰瀑布、斯科加瀑布和黑沙滩";
  if (days.some((d) => d.title === southTitle)) return days;
  if (days[0]?.title !== "入境日") return days;
  const idx = decoded.indexOf('"title":"' + southTitle + '"');
  if (idx < 0) return days;
  const chunk = decoded.slice(idx, idx + 20000);
  const descStart = chunk.indexOf('"description":"') + '"description":"'.length;
  const locMarker = '","location":';
  const descEnd = chunk.indexOf(locMarker, descStart);
  if (descStart < 0 || descEnd < 0) return days;
  const locMatch = chunk
    .slice(descEnd)
    .match(/^","location":(null|"([^"]*)")/);
  if (!locMatch) return days;
  const inserted = [...days];
  inserted.splice(1, 0, {
    title: southTitle,
    description: cleanSlmText(
      chunk.slice(descStart, descEnd).replace(/\\"/g, '"'),
    ),
    location: locMatch[1] === "null" ? "" : (locMatch[2] ?? ""),
  });
  return inserted;
}
...
function renderPackage(meta, dayData) {
  ...
  const itineraryDays = meta.days.map((day, index) => {
    ...
    const hl = `icelandGroupWinter9NonRingDay${dayNum}Highlights`;
    const ex = `icelandGroupWinter9NonRingDay${dayNum}OptionalActivities`;
    return `    {
      day: ${dayNum},
      title: "${esc(title)}",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-winter-9-non-ring-day1-cards.ts"),
  renderExports([
    {
      name: "icelandGroupWinter9NonRingDay1Highlights",
      items: dayData["入境日"].highlights,
    },
    {
      name: "icelandGroupWinter9NonRingDay1OptionalActivities",
      items: dayData["入境日"].extras,
    },
  ]),
);

const DAY2 = "冰岛南岸：塞里雅兰瀑布、斯科加瀑布和黑沙滩";
const DAY3 = "东南部风光：杰古沙龙冰河湖、钻石沙滩、蓝冰洞探险";
const DAY4 = "黄金圈之旅：黄金瀑布、间歇泉、冰岛议会公园";
const DAY5 = "西部小众风景：熔岩瀑布、儿童瀑布、冰岛传奇故乡";
const DAY6 = "斯奈山半岛：网红黑教堂、权游箭头山、阿尔纳斯塔皮渔村";
const DAY7 = "特别浮潜日：世界热门潜水地丝浮拉裂谷浮潜体验";
const DAY8 = "首都自由活动日：探索最北首都的悠闲时光";

fs.writeFileSync(
  path.join(outDir, "iceland-group-winter-9-non-ring-days2-9-cards.ts"),
  renderExports([
    {
      name: "icelandGroupWinter9NonRingDay2Highlights",
      items: dayData[DAY2].highlights,
    },
    {
      name: "icelandGroupWinter9NonRingDay2OptionalActivities",
      items: dayData[DAY2].extras,
    },
    {
      name: "icelandGroupWinter9NonRingDay3Highlights",
      items: dayData[DAY3].highlights,
    },
    {
      name: "icelandGroupWinter9NonRingDay3OptionalActivities",
      items: dayData[DAY3].extras,
    },
    {
      name: "icelandGroupWinter9NonRingDay4Highlights",
      items: dayData[DAY4].highlights,
    },
    {
      name: "icelandGroupWinter9NonRingDay4OptionalActivities",
      items: dayData[DAY4].extras,
    },
    {
      name: "icelandGroupWinter9NonRingDay5Highlights",
      items: dayData[DAY5].highlights,
    },
    {
      name: "icelandGroupWinter9NonRingDay5OptionalActivities",
      items: dayData[DAY5].extras,
    },
    {
      name: "icelandGroupWinter9NonRingDay6Highlights",
      items: dayData[DAY6].highlights,
    },
    {
      name: "icelandGroupWinter9NonRingDay6OptionalActivities",
      items: dayData[DAY6].extras,
    },
    {
      name: "icelandGroupWinter9NonRingDay7Highlights",
      items: dayData[DAY7].highlights,
    },
    {
      name: "icelandGroupWinter9NonRingDay7OptionalActivities",
      items: dayData[DAY7].extras,
    },
    {
      name: "icelandGroupWinter9NonRingDay8Highlights",
      items: dayData[DAY8].highlights,
    },
    {
      name: "icelandGroupWinter9NonRingDay8OptionalActivities",
      items: dayData[DAY8].extras,
    },
    {
      name: "icelandGroupWinter9NonRingDay9Highlights",
      items: dayData["离境日"].highlights,
    },
    {
      name: "icelandGroupWinter9NonRingDay9OptionalActivities",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-winter-9-non-ring.ts"),
