...
const SLM_HTML =
  ...
  path.join(ROOT, "tmp/slm-summer-group-7-south-golden-circle-hiking-live.html");
const PACKAGE_PREFIX = "icelandGroupSummer7SouthGoldenCircleHiking";
const PACKAGE_ID = "iceland-group-summer-7-south-golden-circle-hiking";
...
const DAY_PAIRS = [
  ["入境日", "黄金圈"],
  ["黄金圈", "冰岛南岸：瀑布 & 冰川 & 黑沙滩"],
  ["冰岛南岸：瀑布 & 冰川 & 黑沙滩", "杰古沙龙冰河湖 & 钻石沙滩 "],
  ["杰古沙龙冰河湖 & 钻石沙滩 ", "火山内部探秘 & 出海观鲸 "],
  ["火山内部探秘 & 出海观鲸 ", "自由活动日（可选兰德曼纳劳卡徒步）"],
...
import {
  ...
  ${PACKAGE_PREFIX}Day7Highlights,
  ${PACKAGE_PREFIX}Day7OptionalActivities,
...
${attractions}
  ...
  routeOverviewSubtitle: "黃金圈、南岸、賞鯨火山與徒步自由日夏季跟團動線",
  routeStops: [
${routeStops.join(",\n")}
  ],
  routeMap: {
...
const DAY1 = "入境日";
const DAY2 = "黄金圈";
const DAY3 = "冰岛南岸：瀑布 & 冰川 & 黑沙滩";
const DAY4 = "杰古沙龙冰河湖 & 钻石沙滩 ";
const DAY5 = "火山内部探秘 & 出海观鲸 ";
const DAY6 = "自由活动日（可选兰德曼纳劳卡徒步）";

fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-7-south-golden-circle-hiking-day1-cards.ts"),
  renderExports([
    {
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-7-south-golden-circle-hiking-days2-7-cards.ts"),
  renderExports([
    {
    ...
    {
      name: `${PACKAGE_PREFIX}Day7Highlights`,
      items: dayData["离境日"].highlights,
    },
    {
      name: `${PACKAGE_PREFIX}Day7OptionalActivities`,
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-7-south-golden-circle-hiking.ts"),
