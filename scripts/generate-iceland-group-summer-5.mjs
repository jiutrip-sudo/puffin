...
const SLM_HTML =
  process.env.SLM_HTML ?? path.join(ROOT, "tmp/slm-summer-group-5-live.html");
const IMG_BASE =
...
function renderPackage(meta, dayData) {
  ...
  const itineraryDays = meta.days.map((day, index) => {
    ...
    const hl = `icelandGroupSummer5Day${dayNum}Highlights`;
    const ex = `icelandGroupSummer5Day${dayNum}OptionalActivities`;
    return `    {
      day: ${dayNum},
      title: "${esc(title)}",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-5-day1-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer5Day1Highlights",
      items: dayData["入境日：雷克雅未克 & Sky Lagoon天空之境温泉"].highlights,
    },
    {
      name: "icelandGroupSummer5Day1OptionalActivities",
      items: dayData["入境日：雷克雅未克 & Sky Lagoon天空之境温泉"].extras,
    },
  ]),
);

const DAY2 = "塞里雅兰瀑布 & 斯科加瀑布 & 索尔黑马冰川 & 黑沙滩";
const DAY3 = "杰古沙龙冰河湖 & 钻石沙滩 & 维克镇";
const DAY4 = "斯奈山半岛：教会山 & 海豹沙滩 & 黑教堂";

fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-5-days2-5-cards.ts"),
  renderExports([
    {
      name: "icelandGroupSummer5Day2Highlights",
      items: dayData[DAY2].highlights,
    },
    {
      name: "icelandGroupSummer5Day2OptionalActivities",
      items: dayData[DAY2].extras,
    },
    {
      name: "icelandGroupSummer5Day3Highlights",
      items: dayData[DAY3].highlights,
    },
    {
      name: "icelandGroupSummer5Day3OptionalActivities",
      items: dayData[DAY3].extras,
    },
    {
      name: "icelandGroupSummer5Day4Highlights",
      items: dayData[DAY4].highlights,
    },
    {
      name: "icelandGroupSummer5Day4OptionalActivities",
      items: dayData[DAY4].extras,
    },
    {
      name: "icelandGroupSummer5Day5Highlights",
      items: dayData["离境日"].highlights,
    },
    {
      name: "icelandGroupSummer5Day5OptionalActivities",
...
fs.writeFileSync(
  path.join(outDir, "iceland-group-summer-5.ts"),
